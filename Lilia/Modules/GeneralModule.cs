...
namespace Lilia.Modules
{
    public class GeneralModule : InteractionModuleBase<ShardedInteractionContext>
    {
        ...
        public async Task GeneralBotCommand()
        {
            ...
            var memberCount =
                Context.Client.Guilds.Aggregate<SocketGuild, ulong>(0, (current, guild) => current + Convert.ToUInt64(guild.MemberCount));
            ...
            if (isTopGgRequestSuccess)
            {
                ...
                if (botId != liliaId)
                    componentBuilder
                        .WithButton("Vote for the original one on top.gg", style: ButtonStyle.Link, url: "https://top.gg/bot/884066006115442708",
                            row: 1);
            }
            ...
            DefaultApplications applications = DefaultApplications.Youtube)
        {
            ...
            var invite = await ((SocketGuildUser)Context.User).VoiceState!.Value.VoiceChannel.CreateInviteToApplicationAsync(applications,
                maxUses: null,
