    ...
    public class MusicQueueModule : ApplicationCommandModule
    {
        ...
        public async Task ViewQueueCommand(InteractionContext ctx)
        {
            ...
            if (tracks.Any())
            {
                ...
                var p = interactivity.GeneratePagesInEmbed(names.ToString(),
                    DSharpPlus.Interactivity.Enums.SplitType.Line, embedBuilder);

                await ctx.EditResponseAsync(new DiscordWebhookBuilder()
                    .WithContent(
                        "If you see \"This interaction failed\", just ignore it. I can not find a workaround :D"));

                await ctx.Channel.SendPaginatedMessageAsync(ctx.Member, p, TimeSpan.FromMinutes(5),
                    DSharpPlus.Interactivity.Enums.PaginationBehaviour.WrapAround);
            }
            else
            {
                await ctx.EditResponseAsync(new DiscordWebhookBuilder()
